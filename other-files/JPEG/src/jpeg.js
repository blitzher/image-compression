const drawJpeg = (blocks, w, h, canvas) => {
    ...
    worker.onmessage = (e) => {
        const drawRgb = gpu.createKernel(function (pxMtx) {
                let x = this.thread.x,
                    y = this.thread.y,
                    r = pxMtx[y][x][0] / 255,
                    g = pxMtx[y][x][1] / 255,
    ...
    const
        ...
        toYCbCr = (data2D, size) => gpu
            .createKernel(function (rgbArr) {
                ...
                return [
                    Math.min(Math.max(0, Math.round(0.299 * r + 0.587 * g + 0.114 * b)), 255),
                    Math.min(Math.max(0, Math.round((-0.1687 * r - 0.3313 * g + 0.5 * b) + 128)), 255),
                    Math.min(Math.max(0, Math.round((0.5 * r - 0.4187 * g - 0.0813 * b) + 128)), 255),
                ];
            }, {
                output: [size],
                precision: 'single'
    ...
    return new Promise((resolve, reject) => {
        img.onload = () => {
            ...
            let data = render.getPixels(),
                cutToSize = to2d(to2d(data, 4), img.width).slice(0, img.height - img.height % 8).map(row => row.slice(0, img.width - img.width % 8)),
                compMtx = [0, 1, 2].map(i => toBlocks(to2d(toColorComps(toYCbCr(cutToSize.flat(), cutToSize.flat().length))[i], cutToSize[0].length)).flat());
            ...
            ).then(x => resolve({ out: x, draw: () => drawJpeg(x, img.width, img.height) }));
        };
